...
#define offset_CurrentInGameUIScene (uintptr_t) Il2CppGetMethodOffset(OBFUSCATE("Assembly-CSharp.dll"), OBFUSCATE("COW"), OBFUSCATE("GameFacade"), OBFUSCATE("CurrentInGameUIScene"), 0)
#define offset_UIHudAuxFire_OnBtnDown (uintptr_t) Il2CppGetMethodOffset(OBFUSCATE("Assembly-CSharp.dll"), OBFUSCATE("COW"), OBFUSCATE("UIHudAuxFireController"), OBFUSCATE("OnBtnDown"), 0)
#define offset_UIHudAuxFire_OnBtnUp (uintptr_t) Il2CppGetMethodOffset(OBFUSCATE("Assembly-CSharp.dll"), OBFUSCATE("COW"), OBFUSCATE("UIHudAuxFireController"), OBFUSCATE("OnBtnUp"), 0)
#define offset_UIInGameScene_AuxFireCtrl 0x560
...
static void* GetUIInGameScene() {
    if (!offset_CurrentInGameUIScene) return nullptr;
    return ((void* (*)())(offset_CurrentInGameUIScene))();
}

static void* GetAuxFireController() {
    void* uiScene = GetUIInGameScene();
    if (!uiScene) return nullptr;
    void* auxCtrl = *(void**)((uintptr_t)uiScene + offset_UIInGameScene_AuxFireCtrl);
    return auxCtrl;
}

static float GetRealTimeSinceStartup() {
    if (!offset_get_realtimeSinceStartup) return 0.0f;
    return ((float (*)(void*))(offset_get_realtimeSinceStartup))(nullptr);
}

static float GetRandomTapInterval() {
    static bool seeded = false;
    if (!seeded) {
        srand((unsigned int)(GetRealTimeSinceStartup() * 1000));
        seeded = true;
    }
...
static void FireButton_OnBtnDown(void* controller) {
    if (!controller || !offset_UIHudAuxFire_OnBtnDown) return;
    ((void (*)(void*))(offset_UIHudAuxFire_OnBtnDown))(controller);
}

static void FireButton_OnBtnUp(void* controller) {
    if (!controller || !offset_UIHudAuxFire_OnBtnUp) return;
    ((void (*)(void*))(offset_UIHudAuxFire_OnBtnUp))(controller);
}
...
static void InitializeFireTapper() {
    ...
    fireTapState.lastTapTime = GetRealTimeSinceStartup();
    fireTapState.isButtonDown = false;
    fireTapState.tapInterval = GetRandomTapInterval();
    fireTapState.isActive = true;
}

static void TickFireTapper() {
    if (!fireTapState.isActive) return;
    if (!fireTapState.auxFireController) {
        fireTapState.auxFireController = GetAuxFireController();
        if (!fireTapState.auxFireController) return;
    }
    float currentTime = GetRealTimeSinceStartup();
    float elapsed = currentTime - fireTapState.lastTapTime;
    if (elapsed >= fireTapState.tapInterval) {
        if (fireTapState.isButtonDown) {
            FireButton_OnBtnUp(fireTapState.auxFireController);
            fireTapState.isButtonDown = false;
            fireTapState.tapInterval = GetRandomTapInterval();
        } else {
            FireButton_OnBtnDown(fireTapState.auxFireController);
            fireTapState.isButtonDown = true;
            fireTapState.tapInterval = 0.005f;
        }
        fireTapState.lastTapTime = currentTime;
    }
}

static void StopFireTapper() {
    if (!fireTapState.isActive) return;
    if (fireTapState.isButtonDown && fireTapState.auxFireController) {
        FireButton_OnBtnUp(fireTapState.auxFireController);
        fireTapState.isButtonDown = false;
    }
